...
void read_mb_sensors(){
    ...
    // Read encoders    
    mb_state.left_encoder_delta = (-1) * rc_encoder_read(LEFT_MOTOR);
    ...
    float encoders2meters = (WHEEL_DIAMETER * 3.1415) / (GEAR_RATIO * ENCODER_RES);
    mb_state.left_velocity = encoders2meters * mb_state.left_encoder_delta / DT;
    mb_state.right_velocity = encoders2meters * mb_state.right_encoder_delta / DT; 
